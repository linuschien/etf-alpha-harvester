...
* **評分表純淨原則**：`global_asset_score` 資料表僅儲存通過硬約束之合格候選標的（`is_qualified = true`），由高至低授予連續組內名次（`class_rank = 1, 2, 3...`）。不合格標的於治理日誌留痕，不計算綜合得分，不寫入評分表，確保個人投組模組（P-01, P-02）取得之候選清單 100% 合規可用。
