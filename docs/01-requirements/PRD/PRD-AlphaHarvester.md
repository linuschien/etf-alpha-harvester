...
* **自適應波動度移動停利告警 (US-P02-03)**：
  ...
  * **硬約束前置門檻（去瑣碎化）**：
    1. 累積持有總股數必須滿 1 整張（$Q_{\text{current}} \ge 1,000$ 股）；未滿 1 張不發動停利，避免零股折價滑價與瑣碎小額交易。
    2. 預估已實現獲利金額必須達到最小有效規模 $\text{Realized Gain} \ge \text{MEAT}$ (20,000 TWD)。
  * **動態停利目標與移動追蹤 (Trailing Take-Profit)**：
    * 動態停利目標：$T_i = \text{Clamp}\left(0.75 \times \sigma_{252, i}, \; 10\%, \; 30\%\right)$。
    * 觸發條件：未實現報酬率曾達標 $T_i$，且當前收盤價相較於自達標以來的「波段最高價」，累計回撤幅度 $\ge 8.0\%$（確認動能竭盡與波段頭部確立）。
  * **賣出數量公式（整張出清、零股留種）**：
