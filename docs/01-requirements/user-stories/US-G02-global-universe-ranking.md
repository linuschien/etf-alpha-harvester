...
- **AC1 (核心大盤 Core 判定基準與硬約束)**：
  ...
  - **淘汰規則**：未通過硬約束即判定不合格淘汰（`isQualified = false`），不進入多因子評分與組內排名，**絕不寫入 `global_asset_score` 資料表**。
- **AC2 (動能衛星 Satellite 硬約束過濾)**：
  ...
  - **淘汰規則**：未通過硬約束即判定不合格淘汰（`isQualified = false`），不進入評分排名，**絕不寫入 `global_asset_score` 資料表**。
