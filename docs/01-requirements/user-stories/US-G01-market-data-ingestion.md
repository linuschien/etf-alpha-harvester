...
## US-G01-04：新上市 ETF 基本面與除息日程採集

**身份**：系統排程器 (System Scheduler)

> **As a** 系統排程器，  
> **I want to** 自動追蹤台股新掛牌 ETF 之基本面資料（代碼、名稱、指數、TER、AUM、除息日程與掛牌日期）並納入日常採集，  
> **So that** 系統能以單一資產元資料模型動態掌握新上市 ETF，無需冗餘獨立資料表，一視同仁納入後續客觀評審管線。
...
- **AC2 (一視同仁納入日常採集)**：新掛牌 ETF 自動登錄並採集歷史報價，所有標的一視同仁納入模組 G-02 之月度分類與篩選管線，無需特規通道限制。
...
### 驗收條件 (Acceptance Criteria)
- **AC1 (檢查指標覆蓋與暫停交易豁免)**：守門員必須對當日排程檢查以下 3 項硬性指標：
  1. 所有個人持倉中標的與 G-02 候選標的，當日 `close_price` 必須存在且 $> 0$。**【分割暫停交易豁免】**：若標的登錄於證交所 `TWTAWU` 暫停交易名單（如因減資、分割停止買賣），守門員自動認可其為合法暫停，沿用最後有效收盤價並標記為 `SUSPENDED_LEGITIMATE`，放行下游運算，不觸發熔斷。
  2. FRED 或美國財政部最新基準殖利率數值必須存在，且數值合理落在 $[0.01, 0.20]$ 區間（1% ~ 20%）。
  3. 各標的滾動交易日價格缺漏率嚴格 $= 0\%$（由 Watermark 差量自動回溯補齊）。
- **AC2 (異常熔斷與水位線連動)**：若有任一檢查項未通過，守門員狀態標記為 `HALT`，資料來源水位線（`DataFeedSyncWatermark`）標記為 `HALT`（不推進成功同步日期），強行阻擋下游模組執行，並於戰情室首頁顯示警示橫幅：「*市場數據採集未齊全，量化引擎已安全暫停，請稍後重試或檢查數據源*」。
